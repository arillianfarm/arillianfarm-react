...
const BlogView = () => {
    ...
    useEffect(() => {
        setLoading(true);
        try {
            // 1. Process and set all entries (CRUCIAL for exposing links to crawler)
            ...
            let initialFeaturedBlog = entriesWithSummary[0]; // Default to the first entry

            // --- REACT-SNAP OPTIMIZATION ---
            // Only attempt to find a specific entry from the URL if we are NOT pre-rendering.
            if (!isPreRendering()) {
                const params = new URLSearchParams(location.search);
                const idFromQuery = params.get('articleId');

                if (idFromQuery) {
                    const foundEntry = entriesWithSummary.find(blog => getSlug(blog.entry_subject) === idFromQuery);
                    if (foundEntry) {
                        initialFeaturedBlog = foundEntry;
                    }
                }
            } else {
                console.log("BlogView: Pre-rendering mode active. Skipping complex URL parsing to speed up snapshot.");
            }
            // --- END OPTIMIZATION ---

            setFeaturedBlogEntry(initialFeaturedBlog);
            setError(null);
        } catch (e) {
            setError(e);
    ...
    const handleBlogEntryClick = (entry) => {
        const path = setLinkWithQueryString('blog', entry.entry_subject)

        // Use React Router's navigate function instead of direct history manipulation
        ...
        navigate(pathForNavigate); // Navigates to the new path
                                        ...
                                        <a className="ml-5" style={{float: section.right_side_pic ? 'right': 'left'}}
                                           ...
                                           rel="noopener noreferrer">
                                            <img
                                                ...
                                                alt={`Section image link for ${section.link}`}
                                            />
                                        </a>
                                        ...
                                        <div className="row">
                                            <div className="col-sm-12">
                                                <iframe
                                                    ...
                                                    title={`Video for ${entry.entry_subject} section ${index}`}
                                                >
                                                </iframe>
                ...
                <div className="col-sm-12">
                    <div className="row">
                        ...
                        <div className="col-sm-12 mt-3 text-center">
                            <Comments
                                article_name={featuredBlogEntry && featuredBlogEntry.entry_subject}
                                article_type="blog"
                                pub_date={featuredBlogEntry && featuredBlogEntry.entry_date}
                            />
                        </div>
                    </div>
                </div>
